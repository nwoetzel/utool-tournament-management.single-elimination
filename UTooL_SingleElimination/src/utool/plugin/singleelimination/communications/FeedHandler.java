		...
		if(localName.equalsIgnoreCase("command")) 
		{
			...
			if(tempTag.equals("sendMatchup"))
			{
				...
				for(int i =tempMsg.size()-1;i>-1; i--)
				{
					...
					else if(tempMsg.get(i).getAttr().equals("Parent"))
					{
						parent = tempMsg.get(i).getMsg();						
					}
				...
				handler.handleSendMatchup(Long.parseLong(id), Long.parseLong(matchup), Long.parseLong(parent), teamName1, teamName2, players1, players2, Integer.parseInt(round), table);
			}
			else if(tempTag.equals("sendScore"))
			{
				...
				handler.handleSendScore(Long.parseLong(id), Long.parseLong(matchup), teamName1, teamName2, score1, score2, Integer.parseInt(round));
			}
			...
			else if(tempTag.equals("sendPlayers"))
			{
				...
				handler.handleSendPlayers(id, players);
			...
			else if(tempTag.equals("sendFinalStandings"))
			{
				...
				handler.handleSendFinalStandings(id, players, w, l, rw, rl, s);
			}
			else if(tempTag.equals("sendRoundTimerAmount"))
			{
				...
				handler.handleSendRoundTimerAmount(Long.parseLong(id), time);
			}
			else if(tempTag.equals("sendClear"))
			{
				...
				handler.handleSendClear(Long.parseLong(id));
			}
			else if(tempTag.equals("sendTournamentName"))
			{
				...
				handler.handleSendTournamentName(Long.parseLong(id), name);
			}
			else if(tempTag.equals("sendBeginNewRound"))
			{
				...
				handler.handleSendBeginNewRound(Long.parseLong(id), Integer.parseInt(round));
			}
			else if(tempTag.equals("sendError"))
			{
				...
				handler.handleSendError(Long.parseLong(id), playerid, errorName, errorMessage);
			}
